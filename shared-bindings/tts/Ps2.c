...
//|           import board
//|
//|           kbd = tts.Ps2(board.D10, board.D11)
//|
//|           while len(kbd) == 0:
//|               pass
//|
...
//|         ...
static mp_obj_t tts_ps2_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args)
{
    enum
    {
        ARG_data_pin,
    ...
    tts_ps2_obj_t *self = mp_obj_malloc(tts_ps2_obj_t, &tts_ps2_type);

    common_hal_tts_ps2_construct(self, data_pin, clock_pin);
...
//|         ...
static mp_obj_t tts_ps2_deinit(mp_obj_t self_in)
{
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    common_hal_tts_ps2_deinit(self);
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_1(tts_ps2_deinit_obj, tts_ps2_deinit);

static void check_for_deinit(tts_ps2_obj_t *self)
{
    if (common_hal_tts_ps2_deinited(self))
    {
...
//|         ...
static mp_obj_t tts_ps2_obj___exit__(size_t n_args, const mp_obj_t *args)
{
    mp_check_self(mp_obj_is_type(args[0], &tts_ps2_type));
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(args[0]);
    common_hal_tts_ps2_deinit(self);
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(tts_ps2___exit___obj, 4, 4, tts_ps2_obj___exit__);

//|     def popleft(self) -> int:
//|         """Removes and returns the oldest received byte. When buffer
//|         is empty, raises an IndexError exception."""
//|         ...
static mp_obj_t tts_ps2_obj_popleft(mp_obj_t self_in)
{
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);

    int b = common_hal_tts_ps2_popleft(self);
    if (b < 0)
    {
...
MP_DEFINE_CONST_FUN_OBJ_1(tts_ps2_popleft_obj, tts_ps2_obj_popleft);
...
//|         ...
static mp_obj_t tts_ps2_obj_sendcmd(mp_obj_t self_in, mp_obj_t ob)
{
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);
    mp_int_t cmd = mp_obj_get_int(ob) & 0xff;
    int resp = common_hal_tts_ps2_sendcmd(self, cmd);
    if (resp < 0)
    {
...
MP_DEFINE_CONST_FUN_OBJ_2(tts_ps2_sendcmd_obj, tts_ps2_obj_sendcmd);
...
//|         ...
static mp_obj_t tts_ps2_obj_clear_errors(mp_obj_t self_in)
{
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);

    return MP_OBJ_NEW_SMALL_INT(common_hal_tts_ps2_clear_errors(self));
}
MP_DEFINE_CONST_FUN_OBJ_1(tts_ps2_clear_errors_obj, tts_ps2_obj_clear_errors);
...
static mp_obj_t ps2_unary_op(mp_unary_op_t op, mp_obj_t self_in)
{
    tts_ps2_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);
    uint16_t len = common_hal_tts_ps2_get_len(self);
    switch (op)
    {
...
static const mp_rom_map_elem_t tts_ps2_locals_dict_table[] = {
    // Methods
    {MP_ROM_QSTR(MP_QSTR_deinit), MP_ROM_PTR(&tts_ps2_deinit_obj)},
    {MP_ROM_QSTR(MP_QSTR___enter__), MP_ROM_PTR(&default___enter___obj)},
    {MP_ROM_QSTR(MP_QSTR___exit__), MP_ROM_PTR(&tts_ps2___exit___obj)},
    {MP_ROM_QSTR(MP_QSTR_popleft), MP_ROM_PTR(&tts_ps2_popleft_obj)},
    {MP_ROM_QSTR(MP_QSTR_sendcmd), MP_ROM_PTR(&tts_ps2_sendcmd_obj)},
    {MP_ROM_QSTR(MP_QSTR_clear_errors), MP_ROM_PTR(&tts_ps2_clear_errors_obj)},
};
static MP_DEFINE_CONST_DICT(tts_ps2_locals_dict, tts_ps2_locals_dict_table);

MP_DEFINE_CONST_OBJ_TYPE(
    tts_ps2_type,
    MP_QSTR_Ps2,
    MP_TYPE_FLAG_NONE,
    make_new, tts_ps2_make_new,
    unary_op, ps2_unary_op,
    locals_dict, &tts_ps2_locals_dict);
